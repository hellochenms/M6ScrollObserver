...
- (void)updateNormalColor:(UIColor *)normalColor
    ...
    CGFloat deltaA = [self.selectedColorRGB[3] floatValue] - [self.normalColorRGB[3] floatValue];
    self.deltaRGB = [NSArray arrayWithObjects:@(deltaR), @(deltaG), @(deltaB), @(deltaA), nil];
}
...
- (M6CurrentColors *)currentColorsWithProgress:(CGFloat)progress {
    UIColor *fromColor = [UIColor colorWithRed:[self.normalColorRGB[0] floatValue] + [self.deltaRGB[0] floatValue] * (1 - progress)
                                         ...
                                         alpha:[self.normalColorRGB[3] floatValue] + [self.deltaRGB[3] floatValue] * (1 - progress)];
    ...
    UIColor *toColor = [UIColor colorWithRed:[self.normalColorRGB[0] floatValue] + [self.deltaRGB[0] floatValue] * progress
                                       ...
                                       alpha:[self.normalColorRGB[3] floatValue] + [self.deltaRGB[3] floatValue] * progress];
...
- (NSArray *)colorRGB:(UIColor *)color {
    ...
    if (count == 4) {
        colorRGB = [NSArray arrayWithObjects:@(components[0]), @(components[1]), @(components[2]), @(components[3]), nil];
    } else if (count == 2) {
        colorRGB = [NSArray arrayWithObjects:@(components[0]), @(components[0]), @(components[0]), @(components[1]), nil];
    } else {}
